    ...
    public class SchedulerHostedService : IHostedService, IDisposable
    {
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            foreach (var taskConfig in _taskConfigs)
            {
                try
                {
                    var from = CronExpression.Parse(taskConfig.Expression).GetNextOccurrence(DateTime.UtcNow);
                    var to = CronExpression.Parse(taskConfig.Expression).GetNextOccurrence(from.Value);
                ...
                {
                    _logger.LogError("Handle Scheduler with Exception Error {@Message} {@StackTrace}", ex.Message, ex.StackTrace);
                }
